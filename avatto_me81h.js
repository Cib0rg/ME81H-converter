...
function getDataValue(dpValue) {
    ...
    switch (dpValue.datatype) {
        ...
        case tuya.dataTypes.number:
            return tuya.convertBufferToNumber(dpValue.data);
...
const fz_moes_thermostat_custom = {
        ...
        convert: (model, msg, publish, options, meta) =>
        {
                ...
                const dpValue = firstDpValue(msg, meta);
        const dp = dpValue.dp;
...
*/
                ...
                case dp_avatto_thermostat_scheduling:
                {
                    const tdelim =' -  ';
                        return {weekdays_p1: `${value[0]}:${value[1]}${tdelim}${value[2]}°`,
                                ...
                                thermostat_schedule:`${value[0]}:${value[1]}/${value[2]} ${value[3]}:${value[4]}/${value[5]} ${value[6]}:${value[7]}/${value[8]} ${value[9]}:${value[01]}/${value[11]} ${value[12]}:${value[13]}/${value[14]} ${value[15]}:${value[16]}/${value[17]} ${value[18]}:${value[19]}/${value[20]} ${value[21]}:${value[22]}/${value[23]}`

                              };
                }
                case dpAvattoThermostatOnOff: // system mode (OFF, ON)
                ...
                case dpAvattoThermostatPreset: // preset
                           if(value === 0) return {preset: 'hold'};
                  else if(value === 1) return {preset: 'program'};
                  else return {};

                case dp_avatto_thermostat_heating_setpoint: // temperature setpoint for both heating and cooling
                ...
                case dpAvattoThermostatMinTemp:
                ...
                case dpAvattoThermostatAntiFrozen:
...
const tz_avatto_thermostat_min_temperature=
{
    key: ['min_temperature'],
        convertSet: async (entity, key, value, meta) =>
        {
          await tuya.sendDataPointValue(entity, dpAvattoThermostatMinTemp, value);
        }
};
...
const tz_avatto_thermostat_max_temperature_limit=
{
     key: ['max_temperature_limit'],
        convertSet: async (entity, key, value, meta) =>
        {
          await tuya.sendDataPointValue(entity, dpAvattoThermostatMaxTempLimit, value);
        }
};
...
const tz_avatto_thermostat_antifrozen=
{
        ...
        convertSet: async (entity, key, value, meta) =>
        {
              if(value === "OFF") { await tuya.sendDataPointBool(entity, dpAvattoThermostatAntiFrozen, false);}
     else if(value === "ON")  { await tuya.sendDataPointBool(entity, dpAvattoThermostatAntiFrozen, true); }
        },
...
module.exports =
    {
        fingerprint: [{modelID: 'TS0601', manufacturerName: '_TZE284_ye5jkfsb'}],
        model: 'ME81H',
        vendor: 'AVATTO',
